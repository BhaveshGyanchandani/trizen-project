/**
 * Storage abstraction over Cloudinary (current, primary provider) and
 * legacy GridFS (retained only as a read/delete fallback until the
 * migration script has moved every pre-existing asset). Callers work
 * against `saveUploadedFile` / `deleteUploadedFile` / `openStoredImage`
 * without needing to know which provider a given asset lives in.
 */
...
/** Configures and returns the Cloudinary SDK client, throwing if required credentials are missing. */
...
/** Validates an image file's presence, MIME type, and size before upload. */
...
/**
 * Uploads an image file to Cloudinary and returns the storage metadata to
 * persist on the owning document (Photo, Event cover, or User avatar).
 * MongoDB stores only the metadata returned here; Cloudinary stores the
 * file bytes.
 */
...
/**
 * Deletes the stored image bytes for `asset` from whichever provider it
 * lives in. GridFS remains only as a read/delete fallback while
 * migration runs.
 */
...
/**
 * Opens a readable stream for a stored image's bytes, along with its
 * content type, from whichever provider it lives in. Calling routes
 * perform authorization before proxying the signed asset.
 */
